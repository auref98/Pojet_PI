...
{
	/**
	 * Permet de récupérer une ligne de la table <code>Adresse</code>. <br><br>
	 ...
	 * @return 	un objet de type <code>Adresse</code> si une ligne a été trouvée dans la table adresse<br>
	 *  			null dans le cas contraire
	 */
	@Override
	public Adresse find(int id)
	{
		String query = "select * from adresse where id = ?";									// Définit la requête SQL avec un paramètre
	...
	{
		String query = "delete from adresse where id = ?";										// Définit la requête SQL avec un paramètre
		PreparedStatement ps = null;														// Initialise un objet PreparedStatement pour executer la requête
		boolean resultat = false;															// Initialise la variable qui sera retournée à false (échec) par défaut
